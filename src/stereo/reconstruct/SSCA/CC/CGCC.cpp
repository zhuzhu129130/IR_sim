	...
	double* lG, double* rG )
{
	double clrDiff = 0;
	// three color
    for( int c = 0; c < 3; c ++ ) {//三通道颜色值绝对值差AD，求加和后除3
		double temp = fabs( lC[ c ] - rC[ c ] );
		clrDiff += temp;
	}
	clrDiff *= 0.3333333333;
	// gradient diff
    double grdDiff = fabs( lG[ 0 ] - rG[ 0 ] );//梯度绝对值差
    clrDiff = clrDiff > CG_TAU_1 ? CG_TAU_1 : clrDiff;//TAD
    grdDiff = grdDiff > CG_TAU_2 ? CG_TAU_2 : grdDiff;//TGD
    return CG_ALPHA * clrDiff + ( 1 - CG_ALPHA ) * grdDiff;//TAD和TGD的加权和
}
...
void CGCC::buildCV( const Mat& lImg, const Mat& rImg, const int maxDis, Mat* costVol )
{
    ...
    Sobel( lGray, lGrdX, CV_64F, 1, 0, 1 );//sobel 大小为1时就是求整幅图的梯度
	...
	for( int d = 0; d < maxDis; d ++ ) {
		...
		for( int y = 0; y < hei; y ++ ) {
			...
			for( int x = 0; x < wid; x ++ ) {
                if( x - d >= 0 ) { //如果左图的点在视差为d时，右图有对应点，那么就TAD+TGD，否则，左图点就是边缘，用一个阈值代替右图。
					double* lC = lData + 3 * x;
...
#ifdef COMPUTE_RIGHT
void CGCC::buildRightCV( const Mat& lImg, const Mat& rImg, const int maxDis, Mat* rCostVol )//不支持右图
{
