...
void DVFileStore::ProcessZbuf(unsigned short *buf_recv,cv::Mat& J)
{
    ...
    ss->reproject(ss->disp8u, ss->img1p, point_cloud_ptr);
...
{    
    ...
    vector<ObjectInfo> objectInfos;
    detectNearObject(ss->img1p,ss->depth,objectInfos);
    if (!objectInfos.empty())
    {
        m_ObjectDistance = objectInfos[0].distance; m_ObjectDistance = (int)(m_ObjectDistance * 10000) / 10000.;
        m_ObjectHeight = objectInfos[0].boundRect.height * objectInfos[0].distance / ss->f; m_ObjectHeight = (int)(m_ObjectHeight * 10000) / 10000.;
        m_ObjectWidth = objectInfos[0].boundRect.width * objectInfos[0].distance / ss->f; m_ObjectWidth = (int)(m_ObjectWidth * 10000) / 10000.;
        m_ObjectDisparity = ss->disp8u.at<char>(objectInfos[0].nearest) / 16.;
    }

    cout << m_ObjectDistance << endl;
 ...
 * 功能 : 检测近距目标，输出目标信息序列
 *----------------------------
 * 函数 : DVFileStore::::detectNearObject
 * 访问 : public
 * 返回 : void
 *
 ...
 */
void DVFileStore::detectNearObject(cv::Mat& image, cv::Mat& pointCloud, vector<ObjectInfo>& objectInfos)
{
 ...
 * 参数 : iters	[in]	形态学处理次数
 */
void DVFileStore::imageDenoising( cv::Mat& img, int iters )
{
    cv::Mat pyr = cv::Mat(img.cols/2, img.rows/2, img.type());

    IplImage iplImg = img;
    cvSmooth(&iplImg, &iplImg, CV_GAUSSIAN, 3, 3);	// 平滑滤波
    //cv::GaussianBlur(img,pyr,cv::Size(3,3));

    cv::pyrDown(img, pyr);	// 对平滑后的图像进行二次缩放
    cv::pyrUp(pyr, img);

    cv::erode(img, img, 0, cv::Point(-1,-1), iters);	// 图像腐蚀
    cv::dilate(img, img, 0, cv::Point(-1,-1), iters);	// 图像膨胀
}
 ...
 * 功能 : 生成近距物体信息序列
 *----------------------------
 * 函数 : DVFileStore::parseCandidates
 * 访问 : private
 * 返回 : void
 *
 ...
 * 参数 : objectInfos	[out]	目标信息序列
 */
void DVFileStore::parseCandidates(cv::Mat& objects, cv::Mat& depthMap, vector<ObjectInfo>& objectInfos)
{
    // 提取物体轮廓
    ...
    for( UINT objID = 0; objID < contours.size(); objID++ )
    {
        ...
        if ( area > areaThresh ) // 面积大于 100 像素时，才有效
        {
            ...
            // 填充物体内部轮廓作为掩码区域
            mask = cv::Scalar(0);
            cv::drawContours(mask, contours, objID, cv::Scalar(255), -1);

            // 计算轮廓矩形
            object.boundRect = cv::boundingRect( contour );
            object.minRect = cv::minAreaRect( contour );
 ...
 */
void DVFileStore::showObjectInfo(vector<ObjectInfo>& objectInfos, cv::Mat& outImage)
{
    ...
    {
        //物体中心
        cv::circle(outImage, objectInfos[i].center, 3, CV_RGB(0,0,255), 2);
        ...
        {
            for( int j = 0; j < 4; j++ )
                cv::line( outImage, rect_points[j], rect_points[(j+1)%4], CV_RGB(255,0,0), 4 );
        }
        else
        {
            for( int j = 0; j < 4; j++ )
                cv::line( outImage, rect_points[j], rect_points[(j+1)%4], CV_RGB(255-i*40,i*40,0), 2 );
        }
    }
}
